...
class $className$ControllerSpec extends SpecBase with MockUserAnswersService {
  ...
  "$className$ Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set($className$Page, $className$.values.toSet)
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill($className$.values.toSet), NormalMode)(dataRequest(request), messages(application)).toString
      }
    }
