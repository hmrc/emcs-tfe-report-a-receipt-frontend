...
class $className$ControllerSpec extends SpecBase with MockitoSugar {
  ...
  "$className$ Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set($className$Page, $className$.values.head)
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill($className$.values.head), NormalMode)(dataRequest(request), messages(application)).toString
      }
    }
