...
trait ReferenceDataHttpParser extends BaseConnectorUtils[CnCodeInformationResponse] {
  ...
  def post(url: String, body: CnCodeInformationRequest)
          (implicit hc: HeaderCarrier, ec: ExecutionContext,
           writes: Writes[CnCodeInformationRequest]): Future[Either[ErrorResponse, CnCodeInformationResponse]] =
    http
      .post(url"$url")
      .withBody(Json.toJson(body))
      .execute[Either[ErrorResponse, CnCodeInformationResponse]](ReferenceDataReads, ec)
}
