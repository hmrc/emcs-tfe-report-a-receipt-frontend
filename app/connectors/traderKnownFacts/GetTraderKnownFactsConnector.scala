...
class GetTraderKnownFactsConnector @Inject()(val http: HttpClient,
                                           config: AppConfig) extends TraderKnownFactsHttpParser {

  lazy val baseUrl: String = config.referenceDataBaseUrl

  def getTraderKnownFacts(exciseRegistrationId: String)
                          (implicit headerCarrier: HeaderCarrier,
                           executionContext: ExecutionContext): Future[Either[ErrorResponse, Option[TraderKnownFacts]]] = {
    get(baseUrl + "/oracle/trader-known-facts", exciseRegistrationId)
      .recover {
