...
trait WineOperationsHttpParser extends BaseConnectorUtils[WineOperationsResponse] {
  ...
  def post(url: URL, body: WineOperationsRequest)
          (implicit hc: HeaderCarrier, ec: ExecutionContext,
           writes: Writes[WineOperationsRequest]): Future[Either[ErrorResponse, WineOperationsResponse]] =
    http
      .post(url)
      .withBody(Json.toJson(body))
      .execute[Either[ErrorResponse, WineOperationsResponse]](WineOperationsReads, ec)
}
