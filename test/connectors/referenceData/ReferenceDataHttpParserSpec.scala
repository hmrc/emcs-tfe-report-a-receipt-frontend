...
class ReferenceDataHttpParserSpec extends SpecBase
  ...
  object TestClass {
    implicit val format: OFormat[TestClass] = Json.format
  }

  lazy val httpParser = new ReferenceDataHttpParser[TestClass] {
    override implicit val reads: Reads[TestClass] = TestClass.format
  ...
  "ReferenceDataReads.read(method: String, url: String, response: HttpResponse)" - {
    ...
    "should return a successful response" - {
      ...
      "when valid JSON is returned that can be parsed to the model" in {
        ...
        val httpResponse = HttpResponse(Status.OK, Json.obj("value" -> "my value"), Map())

        httpParser.ReferenceDataReads.read("POST", "/oracle/cn-code-information", httpResponse) mustBe Right(TestClass("my value"))
      }
    }
