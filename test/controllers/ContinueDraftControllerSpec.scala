...
import pages.ContinueDraftPage
...
class ContinueDraftControllerSpec extends SpecBase with MockUserAnswersService {

  def onwardRoute = Call("GET", "/foo")

  val formProvider = new ContinueDraftFormProvider()
  val form = formProvider()

  lazy val continueDraftRoute = routes.ContinueDraftController.onPageLoad(testErn, testArc, NormalMode).url

  "ContinueDraft Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, continueDraftRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswers.set(ContinueDraftPage, true)
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, continueDraftRoute)
