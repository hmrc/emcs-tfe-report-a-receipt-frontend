    ...
    "calling .apply(ern)" - {
      ...
      "User is not logged in" - {
        ...
        "redirect to the sign-in URL with the ContinueURL set" in new Harness {
          ...
          redirectLocation(result) mustBe Some("http://localhost:9949/auth-login-stub/gg-sign-in?continue=http%3A%2F%2Flocalhost%3A8313%2Femcs%2Freport-receipt%2Fern%2Farc")
        }
      }
      ...
      "User is logged in" - {
        ...
        "Affinity Group of user is not Organisation" - {
          ...
          "redirect to unauthorised" in new Harness {
            ...
            status(result) mustBe SEE_OTHER
            redirectLocation(result) mustBe Some(controllers.routes.UnauthorisedController.notAnOrganisation().url)
          }
        }
