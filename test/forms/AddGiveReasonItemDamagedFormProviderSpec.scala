...
class AddGiveReasonItemDamagedFormProviderSpec extends StringFieldBehaviours {

  val maxLength = 350
  val aboveMaxLength = 351

  Seq(
    AddGiveReasonItemDamagedPage(1)
  ).foreach { page =>

    s"loading the form for the '$page' page" - {

      val form = new AddGiveReasonItemDamagedFormProvider()()
